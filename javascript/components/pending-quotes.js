...
export const renderPendingQuotes = () => {
  const container = document.querySelector(
    '[data-component="pending-quotes-list"]'
  );
  container.innerHTML = state.pendingQuotes
    .map(
      (quote) => `
...
export const handleDeleteQuote = (event) => {
  if (event.target.dataset.action === "delete-quote") {
    const quoteId = Number(
      event.target.closest('[data-component="pending-quote"]').dataset.id
    );
    state.pendingQuotes = state.pendingQuotes.filter(
      (pendingQuote) => pendingQuote.id !== quoteId
    );
